...
#
#  FIX 2: simulate_var_b3 now uses the same shock variance
#  formula as generate_shocks:
#    Var(shock k, regime 2) = (1-LAMBDA_FIXED) + LAMBDA_FIXED * lambda_k
#  so that the DGP is fully consistent across all experiments.
#  For the observed shocks (k=1,2,3) this gives lam_obs-scaled
#  variances; for the omitted shock (k=4) it gives
#  (1-LAMBDA_FIXED) + LAMBDA_FIXED * LAMBDA4_B3.
#  The perturbation direction V_B3 is recomputed accordingly.
#
#  ||E_W|| is approximately fixed across lam_obs because
#  P1 = cc' and P2 = [(1-lam) + lam*LAMBDA4_B3]*cc' are
#  independent of lam_obs; lam_obs only affects the observed-
#  shock part of Sigma^(1), which enters C_ and thus E_W only
#  via the whitening, producing a mild residual dependence.
##############################################################
...
# Omitted shock variance in regime 2 — consistent with generate_shocks
...
# j* per lam_obs (Q_tilde changes with lam_obs)
...
# FIX 2: simulate_var_b3 aligned to generate_shocks variance formula
simulate_var_b3 <- function(sc, lam_obs, lambda4, T_tot) {
  # regime-2 std devs: same formula as generate_shocks
  # observed shocks: (1-lam_obs) + lam_obs * c(2,3,4)
  # omitted shock:   (1-LAMBDA_FIXED) + LAMBDA_FIXED * lambda4  [= VAR4_R2_B3]
...
one_rep_b3 <- function(sc, lam_obs, lambda4, T_tot, Q_tilde, jstar) {
  ...
  # sin(theta) for j* only — jstar from Q_tilde (DGP property)
...
# ── Fig B3: fixed T, lam_obs on x-axis ──────────────────────
T_B3_fixed   <- 30000
